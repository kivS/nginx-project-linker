...
if(nginx_server_dir === null) {
  const prompted_nginx_server_dir = prompt('Please enter the path to the nginx server directory', '/opt/homebrew/etc/nginx/servers')
  if(prompted_nginx_server_dir == null) console.error('%cNo nginx server directory provided', ERROR_MESSAGE_STYLE), Deno.exit(1)

  localStorage.setItem('NGINX_SERVER_DIR', prompted_nginx_server_dir)
  console.log(`%cNGINX_SERVER_DIR set to ${prompted_nginx_server_dir}`, SUCCESS_MESSAGE_STYLE)
}

const prompted_file = Deno.args[0] ||
  prompt("Enter the path to the nginx file to be processed:", "nginx.conf");
if (prompted_file == null) console.error("%cwe need a file...", ERROR_MESSAGE_STYLE), Deno.exit(1);
...
if (!nginx_file_lstat.isFile) console.error("%cwe need a file...", ERROR_MESSAGE_STYLE), Deno.exit(1);
...
if (nginx_symlink_save_as == null) {
  console.error("%cwe need to know where to put the file...", ERROR_MESSAGE_STYLE), Deno.exit(1);
}

const nginx_destination_file = `${nginx_server_dir}/${nginx_symlink_save_as}`;

const confirmation = prompt(`About to create the following symbolic link [ ${nginx_destination_file} ] Continue?`, "y");
if (confirmation && ['y' , 'yes', 'ya'].includes(confirmation?.toLowerCase()) == false ) console.log('Aborting...'), Deno.exit(1)
